...
class main() :
    def __init__(self):
        try :
             ...
             StructField('trashedAt', StringType(), True),
             StructField('updatedAt', StringType(), True),
             StructField('websiteUrl', StringType(), True),
             StructField('relationship_assignedUsers', StringType(), True),
             ...
             itr_df = spark.sql("""
                                ...
                                from {OUTREACH_BASE} 
                                where event = 'accounts' and string(to_date(from_dt))  between '{from_dt}' and '{to_dt}'
             ...
             for i in range(1,max_itr+1):
                 ...
                 df_data_explode_level1['data_explode']['attributes']['domain'].cast(StringType()).alias('domain'),
                 df_data_explode_level1['data_explode']['attributes']['industry'].cast(StringType()).alias('industry '),
                 df_data_explode_level1['data_explode']['attributes']['name'].cast(StringType()).alias('name'),
                 df_data_explode_level1['data_explode']['attributes']['named'].cast(StringType()).alias('named'),
                 df_data_explode_level1['data_explode']['attributes']['touchedAt'].cast(StringType()).alias('touchedAt'),
                 df_data_explode_level1['data_explode']['attributes']['trashedAt'].cast(StringType()).alias('trashedAt'),
                 df_data_explode_level1['data_explode']['attributes']['updatedAt'].cast(StringType()).alias('updatedAt'),
                 df_data_explode_level1['data_explode']['attributes']['websiteUrl'].cast(StringType()).alias('websiteUrl'),
                 get_json_object(to_json(struct(df_data_explode_level1['assignedUsers'])),'$.assignedUsers.id').cast(StringType()).alias('relationship_assignedUsers'),
                 ...
                 get_json_object(to_json(struct(df_data_explode_level1['data_explode']['relationships']['assignedUsers']['meta'])),'$.meta.count').cast(StringType()).alias('relationship_assignedUsers_count'),
